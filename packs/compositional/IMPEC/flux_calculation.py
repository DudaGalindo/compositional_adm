...
class RiemannSolvers:
    ...
    def get_extrapolated_properties(self, fprop, M, Nk_face, z_face, P_face, Vp, v, ponteiro):
        ...
        if ctes.compressible_k:
            ...
            rho_j_face1 = np.tile(fprop.rho_j[:,:,self.v0[ponteiro,0]],(int(v/2) * \
            (np.sign(v - ctes.n_components)**2) + int(v)*(1 - np.sign(v -
            ctes.n_components)**2)))
            rho_j_face2 = np.tile(fprop.rho_j[:,:,self.v0[ponteiro,1]],(int(v/2) * \
            (np.sign(v - ctes.n_components)**2) + int(v/(ctes.n_components+1))*(1 -
            np.sign(v-ctes.n_components))))
            Csi_j_face1 = np.tile(fprop.Csi_j[:,:,self.v0[ponteiro,0]],(int(v/2) * \
            (np.sign(v - ctes.n_components)**2) + int(v)*(1 - np.sign(v -
            ctes.n_components)**2)))
            Csi_j_face2 = np.tile(fprop.Csi_j[:,:,self.v0[ponteiro,1]],(int(v/2) * \
            (np.sign(v - ctes.n_components)**2) + int(v/(ctes.n_components+1))*(1 -
            np.sign(v-ctes.n_components))))
            rho_j_face = np.concatenate((rho_j_face1, rho_j_face2), axis=2)
            Csi_j_face = np.concatenate((Csi_j_face1, Csi_j_face2), axis=2)
    ...
    def Fk_from_Nk(self, fprop, M, Nk, P_face, ftotal, ponteiro):
        ...
        f.update_Fj_internal_faces(np.tile(ftotal[:,ponteiro],v), rho_j, mobilities, Pcap_reshaped,
    ...
    def update_flux_LLF(self, Fk_face_LLF_all, Nk_face_LLF, alpha_LLF):
        ...
        alpha_LLF = np.max(abs(alpha),axis=-1)
        Fk_face_LLF = 0.5*(Fk_face_LLF_all.sum(axis=-1) - alpha_LLF * \
                    (Nk_face_LLF[:,:,1] - Nk_face_LLF[:,:,0]))
        return Fk_face_LLF, alpha_LLF
...
class MUSCL:
    ...
    def run(self, M, fprop, wells, P_old, ftot, order=2):
    ...
    def update_flux(self, M, fprop, Nk_face, ftotal):
        ...
        alpha_wv = np.empty((ctes.n_internal_faces))
        Fk_face = RiemannSolvers(ctes.v0).get_Fk_face(fprop, M, Nk_face, self.P_face, self.Ft_internal_faces)
        ...
        Fk_internal_faces[:,~ponteiro], alpha_wv[~ponteiro] = RiemannSolvers(ctes.v0).LLF(M, fprop, Nk_face, self.P_face,
        ...
        Flux().update_flux_volumes(fprop, Fk_internal_faces)
        return alpha_wv
...
class FR:
    ...
    def run(self, M, fprop, wells, Ft_internal_faces, Nk_SP_old, P_old, q, delta_t):

        fprop.Nk_SP = np.copy(Nk_SP_old)

        q_SP = q[:,:,np.newaxis] * np.ones_like(fprop.Nk_SP)

        dFk_SP, wave_velocity = self.dFk_SP_from_Pspace(M, fprop, wells, Ft_internal_faces, fprop.Nk_SP, P_old)
        fprop.Nk_SP = RK3().update_composition_RK3_1(Nk_SP_old, q_SP, dFk_SP, delta_t)
        Nk_SP = np.copy(fprop.Nk_SP)
        fprop.Nk_SP = self.MLP_slope_limiter(Nk_SP)

        dFk_SP, wave_velocity = self.dFk_SP_from_Pspace(M, fprop, wells, Ft_internal_faces, fprop.Nk_SP, P_old)
        fprop.Nk_SP = RK3().update_composition_RK3_2(Nk_SP_old, q_SP, fprop.Nk_SP, dFk_SP, delta_t)
        Nk_SP = np.copy(fprop.Nk_SP)
        fprop.Nk_SP = self.MLP_slope_limiter(Nk_SP)

        dFk_SP, wave_velocity = self.dFk_SP_from_Pspace(M, fprop, wells, Ft_internal_faces, fprop.Nk_SP, P_old)
        fprop.Nk_SP = RK3().update_composition_RK3_3(Nk_SP_old, q_SP, fprop.Nk_SP, dFk_SP, delta_t)
        Nk_SP = np.copy(fprop.Nk_SP)
        fprop.Nk_SP = self.MLP_slope_limiter(Nk_SP)

        fprop.Fk_vols_total = np.min(abs(dFk_SP),axis=2)

        Nk = 1/2*np.sum(ctes_FR.weights * fprop.Nk_SP,axis=2)

        z = Nk[0:ctes.Nc,:] / np.sum(Nk[0:ctes.Nc,:], axis = 0)
        #import pdb; pdb.set_trace()
        if len(fprop.Nk_SP[fprop.Nk_SP<0])>0:
            import pdb; pdb.set_trace()
        return wave_velocity, Nk, z

    def dFk_SP_from_Pspace(self, M, fprop, wells, Ft_internal_faces, Nk_SP, P_old):
        Fk_faces, Nk_face, Fk_vols_RS_neig, wave_velocity = self.Fk_Nk_face(M, fprop, wells,
    ...
    def Fk_Nk_face(self, M, fprop, wells, Ft_internal_faces, Nk_SP, P_old):
        ...
        #Nk_vol_face[:,wells['all_wells']] = fprop.Nk[:,wells['all_wells'],np.newaxis]
        #Nkfunc = lambdify(x, fprop.Nk_SP)
        ...
        Fk_vols_RS_neig, wave_velocity = self.Riemann_Solver(M, fprop, Nk_faces, P_face, Ft_internal_faces, Fk_faces)

        #Fk_vols_RS_neig[:,wells['all_wells'],1] = -Fk_vols_RS_neig[:,wells['all_wells'],1]
        return Fk_faces, Nk_faces, Fk_vols_RS_neig, wave_velocity
    ...
    def Riemann_Solver(self, M, fprop, Nk_faces, P_face, Ft_internal_faces, Fk_faces):
        ...
        return Fk_vols_RS_neig, alpha_wv
    ...
    def MLP_slope_limiter(self, Nk_SP_in):
        Nk_SP = np.copy(Nk_SP_in)
        machine_error = np.finfo(np.float64).eps
        Nk_avg = 1 / 2 * np.sum(ctes_FR.weights * Nk_SP, axis=2)

        inds = np.array([0,-1])
        Nk_SP_vertex = Nk_SP[:,:,inds]

        Nk = (np.linalg.inv(ctes_FR.V)[np.newaxis,] @ Nk_SP[:,:,:, np.newaxis])[:,:,:,0]
        ...
        rs[(Nk_P1_vertex - Nk_P0_vertex)==0] = 1e3
        r = np.max(rs, axis = -1)
        Phi_r = r
        Phi_r[Phi_r>1] = 1
        #Phi_r[Phi_r<0] = 0
        ...
        Phi_P1[Phi_P1==0] = np.min(Phi_r, axis=2)[Phi_P1==0]
        ...
        if ctes_FR.n_points == 3:
            ...
            '''High_order_term = Nk_SP_vertex - Nk_P1_vertex
            Nk_less_max = (Nk_SP_vertex < np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])
            Nk_big_min = (Nk_SP_vertex > np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])
            Nk_less_max[abs(Nk_SP_vertex - np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) < machine_error] = True
            Nk_big_min[abs(Nk_SP_vertex - np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) < machine_error] = True

            C1 = (Linear_term > 0) * (High_order_term < 0) * \
                (Nk_big_min)
            C2 = (Linear_term < 0) * (High_order_term > 0) * \
                (Nk_less_max)
            #C3 = (abs(Nk_SP_vertex - Nk_P0_vertex) <= abs(1e-3*Nk_P0_vertex)) #?
            ...
            C_high_order1 = (Nk_SP_vertex <= np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) #<= machine_error
            C_high_order2 = (Nk_SP_vertex >= np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) #>= -machine_error
            C_high_order1[abs((Nk_SP_vertex - np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])) <= machine_error] = True
            C_high_order2[abs((Nk_SP_vertex - np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])) <= machine_error] = True
            ...
            Nk_SP_lim = (Nk_P0 + Phi_P1[:,:,np.newaxis] * (Nk_P1 - Nk_P0) + \
                phi_Pn[:,:,np.newaxis] * (Nk_SP - Nk_P1))
        else:
            Nk_SP_lim = Nk_P0 + Phi_P1[:,:,np.newaxis] * (Nk_P1 - Nk_P0)

        C_high_order_check1 = (Nk_SP_lim[:,:,inds] <= np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) #<= machine_error
        C_high_order_check2 = (Nk_SP_lim[:,:,inds] >= np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec]) #>= -machine_error
        C_high_order_check1[abs((Nk_SP_lim[:,:,inds] - np.max(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])) <= machine_error] = True
        C_high_order_check2[abs((Nk_SP_lim[:,:,inds] - np.min(Nk_neig, axis = 2)[:,ctes_FR.vols_vec])) <= machine_error] = True
        ...
        #if phi_Pn[0,3]==0: import pdb; pdb.set_trace()
        #import pdb; pdb.set_trace()
        return Nk_SP_lim
