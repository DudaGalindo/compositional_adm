...
class PropertiesCalc:
    ...
    def run_all(self, fprop, kprop):
        self.update_mobilities(fprop, kprop)
    ...
    def update_relative_permeabilities(self, fprop, kprop):
        ...
        relative_permeabilities = np.zeros([1, kprop.n_phases, ctes.n_volumes])
        if kprop.load_k:
            relative_permeabilities[0,0,:] = kro
            relative_permeabilities[0,1,:] = krg
        if kprop.load_w:
            relative_permeabilities[0, kprop.n_phases-1,:] = krw
            #if kprop.load_k:
    ...
    def update_phase_viscosities(self, fprop, kprop):
        phase_viscosities = np.empty([1, kprop.n_phases, ctes.n_volumes])
        if kprop.load_k:
            phase_viscosity = self.phase_viscosity_class(ctes.n_volumes, fprop, kprop)
            ...
            phase_viscosities[0,0:kprop.n_phases-1*kprop.load_w,:] = phase_viscosity(fprop, kprop)
        if kprop.load_w:
            phase_viscosities[0,kprop.n_phases-1,:] = data_loaded['compositional_data']['water_data']['mi_W']
        return phase_viscosities

    def update_mobilities(self, fprop, kprop):
        krs = self.update_relative_permeabilities(fprop, kprop)
        mis = self.update_phase_viscosities(fprop, kprop)
